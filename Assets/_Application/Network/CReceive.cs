...
//-----------------------------------------------------------------------------------------------------------------------------------
public class CReceive : MonoBehaviour
{
	//-------------------------------------------------------------------------------------------------------------------------------
	// -
	//-------------------------------------------------------------------------------------------------------------------------------
    ...
    // 패킷 수신에 반응하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
    public void ON( string compile, AppsParameter col )
    {
	    //---------------------------------------------------------------------------------------------------------------------------
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
        ...
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
		if( ONSystem( (compile), (col) ) ) return;
		if( ONPlayer( (compile), (col) ) ) return;
		if( ONPerson( (compile), (col) ) ) return;
		if( ONCompany( (compile), (col) ) ) return;
		if( ONBusiness( (compile), (col) ) ) return;
		if( ONDepartment( (compile), (col) ) ) return;
		if( ONProduct( (compile), (col) ) ) return;
		if( ONInterface( (compile), (col) ) ) return;
		if( ONStock( (compile), (col) ) ) return;
    ...
    // 플레이어 정보를 수신하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	bool ONPlayer( string compile, AppsParameter col )
    {
	    //---------------------------------------------------------------------------------------------------------------------------
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
	...
	void ApplicationContractAgree( AppsParameter col )
    {
		...
		if( true )
		{
		    ...
		    // -
		    //-----------------------------------------------------------------------------------------------------------------------
			if( app.ViewApplicationContract.Is() || !Func.Is(app.Player.Name()) )
			{
	...
	void IdentityPerson( AppsParameter col )
    {
		...
		if( (person)!=(null) )
		{
			...
			{
				ManagementHash.PersonNation( app.NationStatic.Find(NATION.대한민국) );
			}
...
//			app.ViewIdentityList.Indigator(false);
	...
	void Transfer( AppsParameter col )
    {
		...
		if( (citystatic)!=(null) )
		{
			...
			{
				app.Confirm.ON( Func.Script( app.Language.Get(TEXT.__으로_이동했습니다), app.Language.Get(citystatic.Name()) ) );
			}
	...
	void Foundation( AppsParameter col )
    {
		...
		if( (company)!=(null) )
		{
		    ...
		    // -
		    //-----------------------------------------------------------------------------------------------------------------------
			app.Confirm.ON( Func.Script( app.Language.Get(TEXT.__를_창업했습니다), app.Company.GetName(company) ) );
	...
	void Build( AppsParameter col )
    {
		...
		{
			app.Confirm.ON( Func.Script( app.Language.Get(TEXT.__에__를_건설했습니다), app.Language.Get(citystatic.Name()), app.Language.Get(businessstatic.Name()) ) );
		}
...
//		app.ViewBottom.Refresh();
	...
	void DepartmentCreate( AppsParameter col )
    {
		...
		{
			app.Confirm.ON( Func.Script( app.Language.Get(TEXT.__부가_개설되었습니다), app.Language.Get(departmentstatic.Name()) ) );
		}
	...
	void ProductDispose( AppsParameter col )
    {
		...
		if( (department)!=(null) )
		{
			...
			if( (product)!=(null) )
			{
				...
				if( product.Get()!=(null) )
				{
					app.Confirm.ON( Func.Script( app.Language.Get(TEXT.__의_재고를_처분했습니다), app.Language.Get(product.Get().Name()) ) );
				}
...
//		(play.Day) = col.GetInt("value");	//여유 시간을 주기위해 5분으로 처리함
	...
	void CompanyPicture( AppsParameter col )
    {
		...
		if( (company)!=(null) )
		{
			app.Company.PictureDownload( (company), col.Get("picture"), col.GetLong("picture_upload_time") );
		}
