...
public class CEngine : MonoBehaviour
{
	...
	void OnEnable()
	{
	    ...
	    // -
	    //---------------------------------------------------------------------------------------------------------------------------
		(play.Touch)	= new tagFocus[7];
		for( int i=0; i<(play.Touch.Length); i++ )
		{
			(play.Touch[i])		= new tagFocus();
		}
...
//		app.Purchase.Download( PURCHASE.부활의_깃털 );
    ...
    // 장면을 설정하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public void OFF( SCENE Scene, SCENE nextScene )
	{
        //---------------------------------------------------------------------------------------------------------------------------
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
	...
	public static string GetCachePath( string filepath )
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( Func.IsTexture(filepath) )
		{
			return GetCachePath()+"/"+Library.GetFileName(filepath)+".texture";
		}
		else
		if( Func.IsAssetBundle(filepath) )
		{
	...
	// 맵 캐시 경로를 얻기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public static string GetMapCachePath( float lat, float lng )
	{
        //---------------------------------------------------------------------------------------------------------------------------
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
		int		nLat	= (int)( (lat) * (10000) );
		int		nLng	= (int)( (lng) * (10000) );

        //---------------------------------------------------------------------------------------------------------------------------
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
		return GetMapCachePath()+"/"+(nLat)+"_"+(nLng)+".png";
	}

	//-------------------------------------------------------------------------------------------------------------------------------
    // -
	//-------------------------------------------------------------------------------------------------------------------------------
	public tagProgress EncodeAsync( HttpProtocol http )
	{
	    //---------------------------------------------------------------------------------------------------------------------------
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
        ...
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
		tagProgress		progress	= app.Progress.ON( "암호화를 해제하는중입니다." );
	...
	public IEnumerator EncodeAsync_( HttpProtocol http, tagProgress progress )
	{
		if( http.www==null ) yield break;
		if( http.www.downloadedBytes==null ) yield break;
	    ...
	    //---------------------------------------------------------------------------------------------------------------------------
        // 복호화된 메모리로부터 애셋번들을 로드함
	    //---------------------------------------------------------------------------------------------------------------------------
		tagProgress		uncompression	= app.Progress.ON( "압축을 해제하고 있습니다." );
		http.Progresses.Add( uncompression );

        //---------------------------------------------------------------------------------------------------------------------------
        // -
        //---------------------------------------------------------------------------------------------------------------------------
		byte[]	bytes		= http.www.downloadHandler.data;
        ...
        for( int i=0; i<(bytes.Length); i+=(SYSTEM.TEXTURE_ENCODE_DENSITY) )
        {
			...
			{
				(progress.value)	= Func.Divide( (i), (bytes.Length) );
		...
		(uncompression.operation)	= (request);
		while( !(request.isDone) && (request.progress)<(1) )
		{
			if( ((DateTime.Now.Ticks)-(preTicks))/(10000000f)>=(1f/30) )
			{
				(uncompression.value)	= (request.progress);
				(preTicks)	= (DateTime.Now.Ticks);
				yield return (null);
			}
		}
        ...
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
		(http.assetBundle)		= (request.assetBundle);

	    //---------------------------------------------------------------------------------------------------------------------------
        // -
	    //---------------------------------------------------------------------------------------------------------------------------
		(progress.value)		= (1f);
		(uncompression.value)	= (1f);
	...
	// 위치를 요청함
	//-------------------------------------------------------------------------------------------------------------------------------
    public void Geolocation( Action<object, object> func=(null), object wParam=(null), object lParam=(null) )
    {
        //---------------------------------------------------------------------------------------------------------------------------
        // -
        //---------------------------------------------------------------------------------------------------------------------------
		StartCoroutine( GeolocationEnumerator( (func), (wParam), (lParam) ) );
	...
	// 위치를 요청함
	//-------------------------------------------------------------------------------------------------------------------------------
    IEnumerator GeolocationEnumerator( Action<object, object> func=(null), object wParam=(null), object lParam=(null) )
    {
        //---------------------------------------------------------------------------------------------------------------------------
        // -
        //---------------------------------------------------------------------------------------------------------------------------
        UnityWebRequest request = Internet.Post( "https://www.googleapis.com/geolocation/v1/geolocate?key="+(SYSTEM.GOOGLE_API_KEY) );
...
//		yield return request.SendWebRequest();
		...
		{
            Debug.Log( "위도-경도 획득 실패 : "+(request.error) );
        }
        else
		{
			//-----------------------------------------------------------------------------------------------------------------------
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			AppsParameter col = AppsParameter.Decode(request.downloadHandler.text);
			if( (col)!=(null) )
			{
				AppsParameter location = AppsParameter.Initialize( col.GetNative("location") as Hashtable );
				if( (col)!=(null) )
				{
					//---------------------------------------------------------------------------------------------------------------
					// -
					//---------------------------------------------------------------------------------------------------------------
					app.Engine.SetGeolocation( location.GetFloat("lat"), location.GetFloat("lng") );

					/*
					//---------------------------------------------------------------------------------------------------------------
					// -
					//---------------------------------------------------------------------------------------------------------------
					if( !app.Map.Standard().Is() )
					{
						app.Map.Standard( app.Engine.Lat(), app.Engine.Lng() );
					}
					*/
		...
		if( (func)!=(null) )
		{
			...
			// -
			//-----------------------------------------------------------------------------------------------------------------------
//			if( (lParam)==(null) ) (lParam) = new tagGPS( app.Engine.Lat(), app.Engine.Lng() );
		...
		else
		{
			...
			// -
			//-----------------------------------------------------------------------------------------------------------------------
//			app.Player.Geolocation( app.Engine.Lat(), app.Engine.Lng() );

			//-----------------------------------------------------------------------------------------------------------------------
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			if( Apps.IsLogin() && !Func.Is(play.country) )
			{
//				RPGLifeHash.CountryRequest( app.Engine.Lat(), app.Engine.Lng() );
			}
...
//			Apps.Login( (APPS_LINKAGE.FACEBOOK), app.Facebook.ID(), app.Facebook.Name() );
	...
	public tagFocus GetTouch( TOUCH_INDEX touchIndex )
	{
		...
		switch( touchIndex )
		{
			...
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			case (TOUCH_INDEX.FIRST):
				return (play.Push);

			//-----------------------------------------------------------------------------------------------------------------------
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			case (TOUCH_INDEX.CONTEXT):
				return (play.Context);

			//-----------------------------------------------------------------------------------------------------------------------
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			case (TOUCH_INDEX.WHEEL):
				return (play.Wheel);
			...
			default:
				...
				// -
				//-------------------------------------------------------------------------------------------------------------------
				if( ((int)(touchIndex))<(10) )
				{
					return (play.Touch[(int)(touchIndex)-(3)]);
				}
...
//				app.Confirm.ON( app.Language.Get(TEXT.이미_파티에_가입되어_있는_플레이업니다) );
	...
	public bool IsDragDistance()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( (play.Push.length)>Func.GetDragDistance() )
		{
			return true;
		}
...
//		if( app.ViewGameOver.Is() ) return true;
	...
	public objCanvas GetGlobalUICanvas()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( (app.ViewBottom._Transform().parent)==play.forwardCanvas.Transform() )
		{
