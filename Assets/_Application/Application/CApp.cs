...
public class CApp : FrameworkBehaviour
{
...
//		Download.Task( ViewCharacterSelect );
		*/
		...
		ManagementAppsEventListener.This.Startup();
	}
	...
	void ONLBUTTONDOWN( Vector3 point, TOUCH_INDEX touchIndex )
	{
		...
		while( true )
		{
			...
			if( play.Push.Is() )	play.Push.Reset();
...
//				if( Control.ONLBUTTONDOWN( point ) ) break;
	...
	//-------------------------------------------------------------------------------------------------------------------------------
	void ONLBUTTONUP( Vector3 point, TOUCH_INDEX touchIndex )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		...
		while( true )
		{
			...
			if( Confirm.Is() )
			{
				...
				// -
				//-------------------------------------------------------------------------------------------------------------------
				if( Confirm.ONLBUTTONUP( (point), (touchIndex) ) ) break;
			...
			else
			{
				...
				*/
				...
				// -
				//-------------------------------------------------------------------------------------------------------------------
				if( Panel.ONLBUTTONUP( (point), (touchIndex) ) ) break;
//				if( Control.ONLBUTTONUP( point ) ) break;

				//-------------------------------------------------------------------------------------------------------------------
				// -
				//-------------------------------------------------------------------------------------------------------------------
				if( Browser.ONLBUTTONUP(point) ) break;
...
//			(touch.length)		= (0);
	...
	// 마우스 이동에 반응하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	void ONDRAG( Vector3 point, TOUCH_INDEX touchIndex )
	{	
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		...
		{
			Browser.ONDRAG( (point), (TOUCH_INDEX.FIRST) );
		}
			...
			switch( touch.phase )
			{
				...
				case (TouchPhase.Moved):
					if( (TOUCH_INDEX)(touch.fingerId)==(TOUCH_INDEX.FIRST) )
					{
						...
						ONDRAG( (touch.position), (TOUCH_INDEX)(touch.fingerId) );
						(play.drag_x)	= (int)(touch.position.x);
						(play.drag_y)	= (int)(touch.position.y);
					}
					else
					{
						ONMULTI_TOUCH_DRAG( (touch.position), (TOUCH_INDEX)(touch.fingerId) );
					}
					break;
...
//			if( Panel.ONDELETE() ) break;
		...
		{
			if( Panel.ONRETURN() ) break;
			if( Browser.ONRETURN() ) break;
			break;
		}
	...
	[NonSerialized] public CBgm								Bgm								= null;
	...
	[NonSerialized] public CPlayer							Player							= null;
...
//			Config.친구_검색에서_감추기( Func.IntToBoolean(PlayerPrefs.GetInt(PREFS.친구_검색에서_감추기)) );		//서버에 저장됨
		...
		Bgm								= GetComponentInChildren(typeof(CBgm)) as CBgm;
		...
		Player							= GetComponentInChildren(typeof(CPlayer)) as CPlayer;
