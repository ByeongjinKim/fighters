...
public class Func
{
    ...
    public static int Limit( int min, int max, int value )
    {
	    ...
	    // -
	    //---------------------------------------------------------------------------------------------------------------------------
        (value)     = Mathf.Min( (_max), (value) );
        (value)     = Mathf.Max( (_min), (value) );
...
//		if( (parent)==(null) ) return (null);	//(NULL) 값을 허용함
		...
		{
			Func.SetLayer( (transform), (parent.gameObject.layer) );
		}
	...
	*/
	...
	//-------------------------------------------------------------------------------------------------------------------------------
    public static byte[] EucKr( string text )
    {
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
        byte[]                  encoding    = System.Text.Encoding.Convert( (System.Text.Encoding.UTF8), (charset), System.Text.Encoding.UTF8.GetBytes(text) );
	...
	// 문자열을 얻기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
    public static string GetString( char text )
    {
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
...
//		if( (root)==(null) ) return (null);		//(NULL)값을 허용함
	...
	//-------------------------------------------------------------------------------------------------------------------------------
	public static Vector3 WorldToCanvasPoint( objCamera camera, objCanvas canvas, Vector3 vPos )
	{
        //---------------------------------------------------------------------------------------------------------------------------
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
	    ...
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
		return WorldToCanvasPoint( camera.Camera(), (canvas), (vPos) );
	}

	//-------------------------------------------------------------------------------------------------------------------------------
	// 월드 좌표를 캔버스 좌표로 변환하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public static Vector3 WorldToCanvasPoint( Camera camera, objCanvas canvas, Vector3 vPos )
	{
        //---------------------------------------------------------------------------------------------------------------------------
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
	    ...
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
		(vPos)	= Func.WorldToScreenPoint( (camera), (vPos) );

        //---------------------------------------------------------------------------------------------------------------------------
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
		return Func.ScreenToCanvasPoint( (canvas), (vPos) );
	}

	//-------------------------------------------------------------------------------------------------------------------------------
	// 월드 좌표를 캔버스 좌표로 변환하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public static Vector3 CanvasToWorldPoint( objCanvas canvas, objCamera camera, Vector3 vPos )
	{
        //---------------------------------------------------------------------------------------------------------------------------
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
		if( (canvas)==(null) ) return new Vector3();
		if( (camera)==(null) ) return new Vector3();
		if( camera.Camera()==(null) ) return new Vector3();

        //---------------------------------------------------------------------------------------------------------------------------
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
		return CanvasToWorldPoint( (canvas), camera.Camera(), (vPos) );
	}

	//-------------------------------------------------------------------------------------------------------------------------------
	// 월드 좌표를 캔버스 좌표로 변환하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public static Vector3 CanvasToWorldPoint( objCanvas canvas, Camera camera, Vector3 vPos )
	{
        //---------------------------------------------------------------------------------------------------------------------------
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
	    ...
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
		(vPos)	= CanvasToScreenPoint( (canvas), (vPos) );

        //---------------------------------------------------------------------------------------------------------------------------
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
		return ScreenToWorldPoint( (camera), (vPos) );
	}

	//-------------------------------------------------------------------------------------------------------------------------------
	// 캔버스 좌표를 스크린 좌표로 변환하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public static Vector3 CanvasToScreenPoint( objCanvas canvas, Vector3 vPos )
	{
        //---------------------------------------------------------------------------------------------------------------------------
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
	    ...
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
		(vPos.x)	= Func.Divide( (vPos.x) + (canvas._RectTransform().sizeDelta.x/2), (canvas._RectTransform().sizeDelta.x) ) * (Screen.width);
		(vPos.y)	= Func.Divide( (vPos.y) + (canvas._RectTransform().sizeDelta.y/2), (canvas._RectTransform().sizeDelta.y) ) * (Screen.height);
	...
	//-------------------------------------------------------------------------------------------------------------------------------
	public static Vector3 ScreenToCanvasPoint( objCanvas canvas, Vector2 point )
	{
	    //---------------------------------------------------------------------------------------------------------------------------
	    // -
	    //---------------------------------------------------------------------------------------------------------------------------
		...
		if( RectTransformUtility.ScreenPointToWorldPointInRectangle( canvas._RectTransform(), (point), canvas.Camera(), out vector ) )
		{
			return (vector);
		}
	...
	//-------------------------------------------------------------------------------------------------------------------------------
	// 마우스 좌표를 캔버스 좌표로 변환하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public static Vector2 MouseToCanvasPoint( objCanvas canvas, Vector2 point )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		(vResult.x)		= Func.Divide( (point.x), (Screen.width) )	* canvas.Width()	- canvas.Width()/(2);
		(vResult.y)		= Func.Divide( (point.y), (Screen.height) ) * canvas.Height()	- canvas.Height()/(2);
	...
	// 월드 좌표를 캔버스 좌표로 변환하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public static Vector3 ScreenToWorld( objCamera camera, Vector3 vPos )
	{
        //---------------------------------------------------------------------------------------------------------------------------
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
	    ...
	    // -
		//---------------------------------------------------------------------------------------------------------------------------
		return ScreenToWorldPoint( camera.Camera(), (vPos) );
	}
	*/
...
//		if( (parent)==(null) ) return (null);	//(NULL)값을 허용함
	...
	// -
	//-------------------------------------------------------------------------------------------------------------------------------
	public static string PostPosition_( string text, string postposition )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( (text)==(null) ) return (postposition);
		if( (postposition)==(null) ) return (null);
		...
		for( int i=(length-1); i>=(0); i-- )
		{
		    ...
		    // -
		    //-----------------------------------------------------------------------------------------------------------------------
			(bytes)		= System.Text.Encoding.Unicode.GetBytes( text.Substring(i,1) );
	...
	// -
	//-------------------------------------------------------------------------------------------------------------------------------
	public static bool IsHttpProtocal( string Url )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( !Func.Is(Url) ) return false;
		...
		//---------------------------------------------------------------------------------------------------------------------------
		(Length)	= ("http://".Length);
		if( (Url.Length)>=(Length) && Url.Substring( (0), (Length) )=="http://" )
		{
			return true;
		}
		...
		//---------------------------------------------------------------------------------------------------------------------------
		(Length)	= ("https://".Length);
		if( (Url.Length)>=(Length) && Url.Substring( (0), (Length) )=="https://" )
		{
			return true;
		}
	...
	// -
	//-------------------------------------------------------------------------------------------------------------------------------
	public static bool IsJarUrl( string Url )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( (Url.Length)>=(Length) && Url.Substring( (0), (Length) )=="jar:file://" )
		{
			return true;
		}
	...
	public static bool IsDesktop()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( Func.IsEditor() || Func.IsWindows() )
		{
			return true;
		}
	...
	public static void CreateDirectory( string path )
	{
		...
		foreach( string dir in pathArray )
		{
			...
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			if( Func.Is(path) && !Directory.Exists(path) )
			{
...
//		CApp.This.MessageQueue.ON( (function.func), (function.wParam), (function.lParam) );
	...
	public static void funcActiveScene( object wParam=(null), object lParam=(null) )
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( ( (wParam)==(null) || wParam.GetType()!=typeof(SceneLevel) ) && Func.Is((string)(lParam)) ) 
		{
			//-----------------------------------------------------------------------------------------------------------------------
			// -
			//-----------------------------------------------------------------------------------------------------------------------
...
//		if( (var3)==(null) ) return (text);		//(NULL)값을 허용함
	...
	public static float GetAndroidOSVersion()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( IsAndroid() && (fAndroidOSVersion)==(-1) )
		{
