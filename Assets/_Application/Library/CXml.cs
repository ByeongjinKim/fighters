...
public class CXml
{
    ...
    public static XmlDocument Load( string filepath )
    {
	    ...
	    // -
	    //---------------------------------------------------------------------------------------------------------------------------
        if( Func.IsBlank(filepath) ) return (null);
...
//          Debug.Log("CXml::Load( "+(xmlFile.name)+" ) - Loaded");
	...
	// 애셋번들 정보를 불러오기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
    public static void GetChildValue( XmlNode pNode, tagAssetBundle assetBundle, string column )
    {
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		...
		if( (pNodeAssetBundle)==(null) ) return;
	...
	//텍스쳐 정보를 불러오기 위한 함수
    public static string GetChildValue( XmlNode pNode, tagTexture texture, string path )
    {
        if( pNode==null ) return null;
        if( texture==null ) return null;
        if( !Library.Is(path)==null ) return null;
		...
		if( pChild!=null )
		{
			...
			texture.cache		= GetChildInt( pChild, "cache" );
	...
	*/
    ...
    public static Quaternion convertQuaternion( string text )
    {
	    ...
	    // -
	    //---------------------------------------------------------------------------------------------------------------------------
        if( (values.Length)>=(1) && Func.IsSingle(values[0]) )
        {
            (quater.x)      = Convert.ToSingle(values[0]);
        }
        if( (values.Length)>=(2) && Func.IsSingle(values[1]) )
        {
            (quater.y)      = Convert.ToSingle(values[1]);
        }
        if( (values.Length)>=(3) && Func.IsSingle(values[2]) )
        {
            (quater.z)      = Convert.ToSingle(values[2]);
        }
        if( (values.Length)>=(3) && Func.IsSingle(values[3]) )
        {
