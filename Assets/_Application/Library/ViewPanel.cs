...
//		if( (data)==(null) ) return false;	//(NULL)값을 허용함
		...
		if( Panel().IsAllowSound() )
		{
			...
			{
				app.MessageQueue.ONAndSkip( (funcSound), (SOUND.팝업) );
			}
		}
...
//			tagHistory history = app.History.GetForeHistory();
    ...
    public virtual bool ONCANCEL( CANCEL Cancel )
    {
		...
		if( Is() )
		{
			...
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			if( Panel().Close()!=(null) && Panel().Close().IsActive() )
			{
				//-------------------------------------------------------------------------------------------------------------------
				// -
				//-------------------------------------------------------------------------------------------------------------------
	...
	//-------------------------------------------------------------------------------------------------------------------------------
	public Transform Loader( UnityEngine.Object sample, objCanvas canvas )
	{
	    //---------------------------------------------------------------------------------------------------------------------------
	    // -
	    //---------------------------------------------------------------------------------------------------------------------------
	...
	public Transform Loader( UnityEngine.Object sample, Transform parent )
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		return Loader( (sample as GameObject), (parent) );
	}

	//-------------------------------------------------------------------------------------------------------------------------------
	// 인터페이스 정보를 불러오기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public Transform Loader( GameObject sample, Transform parent )
	{
	    //---------------------------------------------------------------------------------------------------------------------------
	    // -
	    //---------------------------------------------------------------------------------------------------------------------------
		...
		(transform.localScale)			= (sample.transform.localScale);
		...
		{
			RectTransform	sampleTransform		= ((sample as GameObject).GetComponent(typeof(RectTransform)) as RectTransform);
			if( (sampleTransform)!=(null) )
			{
//				rectTransform.SetParent( canvas.RectTransform() );
				(rectTransform.anchoredPosition)	= (sampleTransform.anchoredPosition);
			}
		}
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		Component[]		comArray	= transform.GetComponentsInChildren(typeof(UnityEngine.UI.Graphic));
		foreach( UnityEngine.UI.Graphic graphic in comArray )
		{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		(Data()._panel)	= (obj.GetComponent(typeof(objPanel)) as objPanel);
		if( Panel()!=(null) )
		{
			//-----------------------------------------------------------------------------------------------------------------------
			// -
...
//		if( (lParam)==(null) ) return;	//(NULL)값을 허용함
	...
	public void Indigator( bool option=(true), string description=(null), int index=(0) )
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
//		if( !Func.Is(description) ) return;		//(NULL)값을 허용함
...
//		if( (history)==(null) ) return;		//(NULL)값을 허용함
	...
	public void funcSound( object wParam=(null), object lParam=(null) )
	{
		...
		{
			app.ViewIndigator.SetCallback( (app.Sound.funcSound), (wParam), (lParam) );
		}
		else
		{
...
//		if( (soundstatic)==(null) ) return;		//(NULL)값을 허용함
	...
	protected void TransNavigation( GameObject gameObject, bool isActive=(true) )
	{
		...
		if( isActive )
		{
			...
			// -
			//---------------------------------------------------------------------------------------------------------------------------
			transform.SetParent( app.ViewNavigation._Transform() );
