...
//		Debug.Log( "Loading-OFF => "+(component) );
	...
	void Update()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		data.progress.Set( Func.Divide( GetTaskProgress(), (data.Tasks.Count) ) );
		
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		Vector3		vParticle		= Func.CanvasToWorldPoint( (play.foreCanvas), play.foreCanvas.Camera(), GetLoadingBarPosition() );
...
//			Debug.Log( component+" => "+instance.description );
	...
	public string GetDescription()
	{
		...
		foreach( tagLoadingInstance instance in Instances() )
		{
			if( Func.Is(instance.description) )
			{
	...
	public Vector3 GetLoadingBarPosition()
	{
		...
		(Value)	+= (data.LoadingRectTransform.sizeDelta.x) * data.progress.Value();
	...
	*/
	...
	public override void Loader()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		base.Loader( Resources.Load("ApplicationLoader/View/ViewLoading"), (play.foreCanvas) );

		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		(data.description)				= (Func.Get( _Transform(), "Description" ).GetComponent(typeof(Text)) as Text);
		(data.progress)					= (Func.Get( _Transform(), "Loading/Value" ).GetComponent(typeof(uiValue)) as uiValue);
		(data.LoadingRectTransform)		= (Func.Get( _Transform(), "Loading" ) as RectTransform);
		(data.title)					= (Func.Get( _Transform(), "Title" ).gameObject);
		(data.rating)					= (Func.Get( _Transform(), "Rating" ).gameObject);
