...
public class CViewNavigation : ViewPanel
{
	...
	public bool ON( ViewPanel panel, string text, Color textColor, Color backColor, float margin=(0) )
	{
		...
		{
			_Transform().SetSiblingIndex( panel._Transform().GetSiblingIndex()+(1) );
		}
...
//				history.panel.ONNAVIGATION();
	...
	public override void Loader()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		base.Loader( Resources.Load("ApplicationLoader/View/ViewNavigation"), app.Engine.GetGlobalUICanvas() );

		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		(data.back)		= (_Transform().GetComponent(typeof(Image)) as Image);
		(data.cancel)	= (Func.Get( _Transform(), "Cancel" ).GetComponent(typeof(objControl)) as objControl);
		(data.subject)	= (Func.Get( _Transform(), "Subject" ).GetComponent(typeof(uiText)) as uiText);
		(data.edit)		= (Func.Get( _Transform(), "Edit" ).GetComponent(typeof(objControl)) as objControl);
