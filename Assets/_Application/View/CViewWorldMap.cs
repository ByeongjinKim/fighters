...
//			app.Canvas.FullscreenScrollViewInit( ScrollView()._RectTransform() );
			...
			if( WorldMap()!=(null) )
			{
				...
				{
					if( app.City.IsTransfer(citystatic) && Func.Is(citystatic.tag) )
					{
	...
	*/
	...
	public uiWorldMapCityPin Register( Vector2 vPos, tagCityStatic citystatic )
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		uiWorldMapCityPin pin = (Func.Create( _Transform(), (Resources.Load("UI/WorldMapCityPin") as GameObject) ).GetComponent(typeof(uiWorldMapCityPin)) as uiWorldMapCityPin);
		if( (transform)==(null) ) return (null);

		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		(pin.Transform().position)			= Func.WorldToCanvasPoint( WorldMap().Camera(), Canvas(), new Vector2( (vPos.x)*(WorldMap().Transform().localScale.x), (vPos.y)*(WorldMap().Transform().localScale.y) ) );
...
//		app.CityStatic.CorverTextureDownload( (citystatic), (pin.funcTextureDownload) );
	...
	public void Refresh()
	{
		...
		if( Airplane()!=(null) )
		{
			(Airplane().Image().position) = Func.WorldToCanvasPoint( WorldMap().Camera(), Canvas(), new Vector2( (Airplane().Transform().position.x), (Airplane().Transform().position.y) ) );
		}
	...
	public float GetZoomScale()
	{
		...
		{
			return Func.Divide( WorldMap().Camera().GetFieldOfView(), (SYSTEM.ZOOM_MAX) );
		}
...
//		(transform.localPosition)		= (app.City.Get().pin.data.city.unity2DLocation);
	...
	public void CreateImage( objAirplane airplane )
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		Transform transform = Func.Create( _Transform(), (Resources.Load("Objects/Airplane") as GameObject) );
		if( (transform)==(null) ) return;

		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		(transform.position)			= Func.WorldToCanvasPoint( WorldMap().Camera(), Canvas(), (airplane.Transform().position) );
		(transform.localEulerAngles)	= new Vector3();
