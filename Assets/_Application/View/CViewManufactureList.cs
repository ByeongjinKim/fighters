...
public class CViewManufactureList : ViewPanel
{
	...
	void funcInitialize( object wParam=(null), object lParam=(null) )
	{
		...
		if( (사육부)!=(null) && 사육부.Make()!=(null) )
		{
			...
			if( (animal)!=(null) )
			{
				...
				foreach( tagProductStatic productstatic in app.ProductStatic.GetList() )
				{
					if( Func.Is(productstatic.resource) && ( (productstatic.resource)==(animal.id) || productstatic.resource.IndexOf("/"+(animal.id)+"/")>=(0) ) )
					{
	...
	void funcManufacture( object wParam=(null), object lParam=(null) )
	{
		...
		ManagementHash.Manufacture( app.Business.Get(), (wParam as string), (lParam as tagProductStatic) );
	...
	public override bool ONCONTROL( objControl control, CONTROL_ACTION Action, tagFocus focus )
	{
		...
		foreach( uiIcon icon in GetList() )
		{
			...
			{
				app.Confirm.ON( Func.Script( app.Language.Get(TEXT.__를_가공하시겠습니까), app.Language.Get(app.ProductStatic._GetName(icon.Get())) ), (CONFIRM.YESNO), (funcManufacture), app.Department.GetID(), icon.Get() );
				return true;
			}
