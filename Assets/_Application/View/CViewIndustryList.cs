...
public class CViewIndustryList : ViewPanel
{
	...
	void funcIndustryRegister( object wParam=(null), object lParam=(null) )
	{
		...
		//---------------------------------------------------------------------------------------------------------------------------
		Indigator( app.Language.Get(TEXT.생산_라인을_가동중입니다) );
		ManagementHash.IndustryRegister( app.Business.Get(), (wParam as string), (lParam as tagProductStatic) );
	...
	bool IsIndustry( tagProductStatic productstatic )
	{
		...
		if( Func.Is(productstatic.resource) )
		{
			...
			{
				if( Func.Is(id) && !app.Department.IsDepot(id) )
				{
					return false;
				}
			}
	...
	public override bool ONCONTROL( objControl control, CONTROL_ACTION Action, tagFocus focus )
	{
		...
		foreach( uiIcon icon in GetList() )
		{
			...
			{
				app.Confirm.ON( Func.Script( app.Language.Get(TEXT.__를_생산하시겠습니까), app.Language.Get(app.ProductStatic._GetName(icon.Get())) ), (CONFIRM.YESNO), (funcIndustryRegister), app.Department.GetID(), icon.Get() );
				return true;
			}
