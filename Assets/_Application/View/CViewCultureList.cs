...
public class CViewCultureList : ViewPanel
{
	...
	void funcCulture( object wParam=(null), object lParam=(null) )
	{
		...
		ManagementHash.Cultivate( app.Business.Get(), (wParam as string), (lParam as tagProductStatic) );
	...
	public override bool ONCONTROL( objControl control, CONTROL_ACTION Action, tagFocus focus )
	{
		...
		foreach( uiIcon icon in GetList() )
		{
			...
			{
				app.Confirm.ON( Func.Script( app.Language.Get(TEXT.__를_재배하시겠습니까), app.Language.Get(app.ProductStatic._GetName(icon.Get())) ), (CONFIRM.YESNO), (funcCulture), app.Department.GetID(), icon.Get() );
				return true;
			}
