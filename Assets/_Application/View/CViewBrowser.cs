...
public class CViewBrowser : ViewPanel
{
	...
	public bool ON( string Url, string indigatorDescripton=(null), bool instantly=(true) )
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( Func.IsBlank(Url) ) return false;
//		if( Func.IsBlank(indigatorDescripton) ) return false;	//(NULL)값을 허용함
		...
		if( base.ON(data) )
		{
			...
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			(_RectTransform().sizeDelta)	= new Vector2( tagSystem.GetScreenWidth()-(60), (_RectTransform().sizeDelta.y) );
...
//			Func.Active( data.webbrowser.gameObject );
	...
	public override void Loader()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		(data.webbrowser)		= (_Transform().GetComponentInChildren(typeof(WebBrowser)) as WebBrowser);

		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		(data.purchase)			= Func.Get( _Transform(), "Purchase" );
