...
public class Apps
{
	...
	//서버에 연결하기 위한 함수
	public static void Connect( Transform transform, string serveraddr, string app, string root )
	{
		if( transform==null ) return;
		if( !Library.Is(serveraddr) ) return;
		if( !Library.Is(app) ) return;
		if( !Library.Is(root) ) return;

		{
			Data().serverAddr = serveraddr;
			if( !Data().serverAddr.StartsWith("http://") && !Data().serverAddr.StartsWith("https://") )
			{
				Data().serverAddr = "https://"+Data().serverAddr;
			}

			if( !Data().serverAddr.EndsWith("/") )
			{
				Data().serverAddr += "/";
			}

			//서버 루트 URL을 얻음
			Data().serverRoot	= GetRoot(Data().serverAddr);
			Data().Root			= root;
		}

		(Data().httpConnect)				= (Data().serverAddr)+"Connect.php";
		(Data().httpConnectAuthenticode)	= (Data().serverAddr)+"ConnectAuthenticode.php";
		(Data().httpAccount)				= (Data().serverAddr)+"Account.php";
		(Data().httpLogin)					= (Data().serverAddr)+"Login.php";
		(Data().httpScore)					= (Data().serverAddr)+"Score.php";
		(Data().httpPurchase)				= (Data().serverAddr)+"Purchase.php";
		(Data().httpGetCash)				= (Data().serverAddr)+"GetCash.php";
		(Data().httpStorageIn)				= (Data().serverAddr)+"StorageIn.php";
		(Data().httpStorageOut)				= (Data().serverAddr)+"StorageOut.php";
		(Data().httpMarketLogin)			= (Data().serverAddr)+"MarketLogin.php";
		(Data().httpMemorize)				= (Data().serverAddr)+"Memorize.php";
		(Data().httpGetMemorize)			= (Data().serverAddr)+"GetMemorize.php";
		(Data().httpLocalizeRegister)		= (Data().serverAddr)+"LocalizeRegister.php";
		(Data().httpLocalizeLogin)			= (Data().serverAddr)+"LocalizeLogin.php";
		(Data().httpUpdate)					= (Data().serverAddr)+"Update.php";
		(Data().httpRank)					= (Data().serverAddr)+"Rank.php";
		(Data().httpLoadRank)				= (Data().serverAddr)+"LoadRank.php";
		(Data().httpVersion)				= (Data().serverAddr)+"Version.php";
		(Data().httpBuy)					= (Data().serverAddr)+"Buy.php";
		(Data().httpGetPlatformInfo)		= (Data().serverAddr)+"GetPlatformInfo.php";
		(Data().httpSendFriends)			= (Data().serverAddr)+"SendFriends.php";
		(Data().httpLoadFriend)				= (Data().serverAddr)+"LoadFriend.php";
		(Data().httpPoint)					= (Data().serverAddr)+"Point.php";
		(Data().httpCash)					= (Data().serverAddr)+"Cash.php";
	...
	public static void Connect()
	{
		...
		{
			post.Register( "app", AppsFunc.encode(Data().app) );
	...
	// 플랫폼을 설정하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public static void SetVersion( float value )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
	...
	public static void Receive( AppsParameter col )
	{
...
#if UNITY_EDITOR
			if( col.www!=null && col.GetInt("LastTime")<=(0) )
			{
				Debug.Log("로컬 업데이트 정보를 갱신했습니다.");

				string filepath = Application.streamingAssetsPath+"/Update.json";
				File.WriteAllText( filepath, col.www.downloadHandler.text );
			}
#endif
			return;
		}
		...
		//---------------------------------------------------------------------------------------------------------------------------
		if( AppsFunc.IsEquals((compile), "UpdateEnd") )
		{
			//-----------------------------------------------------------------------------------------------------------------------
			// -
			//-----------------------------------------------------------------------------------------------------------------------
	...
	//-------------------------------------------------------------------------------------------------------------------------------
	public static void InitHTTP( Transform transform )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		...
		if( (Data().http)==(null) )
		{
			...
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			if( transform.GetComponent(typeof(AppsHTTP)) )
			{
				(Data().http)  = (transform.GetComponent(typeof(AppsHTTP)) as AppsHTTP);
			}
			else
			{
				(Data().http)  = (transform.gameObject.AddComponent(typeof(AppsHTTP)) as AppsHTTP);
			}
...
//      if( post==null ) return; //(NULL)값을 허용함
		...
		Data().http.Post( Data().serverAddr+file+".php", post );
	}
...
//      if( post==null ) return; //(NULL)값을 허용함
		...
		Data().http.Post( Data().serverRoot+Data().Root+"/"+file+".php", post );
	}
	...
	// 아이템 정보를 얻기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	public static void Update( int LastUpdateTime )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		...
		Data().http.Post( (Data().httpUpdate), (post) );
	...
	static void Version( AppsParameter col )
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( !(Application.isEditor) && LoadUpdateTime()<=(0) )
		{
			LocalUpdate();
		}
		else
		if( (Application.isEditor) && IsNotUpdateJson() )
		{
			//-----------------------------------------------------------------------------------------------------------------------
			// -
			//-----------------------------------------------------------------------------------------------------------------------
		...
		//---------------------------------------------------------------------------------------------------------------------------
		// 어플리케이션 버전이 다를 경우, 로컬 패치로만 업데이트될 것이므로, 서버를 통한 전체 업데이트는 진행하지 않음
		//---------------------------------------------------------------------------------------------------------------------------
		if( (Data().fVersion)>=col.GetFloat("Version") && LoadVersion()!=col.GetFloat("Version") )
		{
			//-----------------------------------------------------------------------------------------------------------------------
			// -
			//-----------------------------------------------------------------------------------------------------------------------
		...
		//---------------------------------------------------------------------------------------------------------------------------
		// 새로 설치된 앱, 로컬 패치로만 업데이트될 것이므로, 서버를 통한 전체 업데이트는 진행하지 않음
		//---------------------------------------------------------------------------------------------------------------------------
		if( !(Application.isEditor) && File.GetLastWriteTime(Application.dataPath).ToString()!=LoadSetupVersion() )
		{
			//-----------------------------------------------------------------------------------------------------------------------
			// -
			//-----------------------------------------------------------------------------------------------------------------------
//			CApp    app     = (GameObject.FindObjectOfType(typeof(CApp)) as CApp);
//			app.Confirm.ON("Test : 앱이 처음 설치되었거나 다시 설치되었으므로 전체 데이타를 서버로부터 동기화합니다.\r\n\r\n기존 버전의 설치된 날짜 ("+LoadSetupVersion()+")\r\n새 버전의 설치된 시간 ("+File.GetLastWriteTime(Application.dataPath).ToString()+")");

			//-----------------------------------------------------------------------------------------------------------------------
			// 앱이 새로 설치되었거나 다시 설치되었을 경우, 전체 데이타를 서버로부터 동기화 함
			//-----------------------------------------------------------------------------------------------------------------------
...
//			Apps.Update( 0 );
		...
		// 버전이 틀립니다.
		//---------------------------------------------------------------------------------------------------------------------------
		if( (Data().fVersion)!=col.GetFloat("Version") )
		{
	...
	{
		if( Platform.IsEditor() && !Library.IsFile(Application.streamingAssetsPath+"/Update.json") )
		{
			return true;
		}
	...
	public static bool eventItemState( AppsParameter col )
	{
		...
		foreach( AppsItemListener listener in Data().AppsItemListeners )
		{
			...
			// -
			//-----------------------------------------------------------------------------------------------------------------------
			if( Equals(listener.ClassName(), col.Get("class")) && ( Equals(listener.TypeName(), col.Get("type")) || Func.IsBlank(listener.TypeName()) ) )
			{
	...
	public static void Cash( int cash )
	{
		...
		AppsHTTPPost    post    = new AppsHTTPPost();
		{
			post.Register( "cash", (cash.ToString()) );
		}
	...
	public static int LoadUpdateTime()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( !File.Exists(GetLastUpdateTimeFilePath()) ) return (0);
		...
		if( (streamReader)!=(null) )
		{
			(UpdateTime)	= Convert.ToInt32(streamReader.ReadToEnd());
		}
		streamReader.Close();
	...
	{
		if( lParam==null || lParam.GetType()!=typeof(HttpProtocol) ) return;

		HttpProtocol http = lParam as HttpProtocol;
		if( http!=null )
		{
	...
	public static void Download( string url )
	{
		...
		AppsHTTPPost    post    = new AppsHTTPPost();
		{
			post.Register( "url",			WWW.EscapeURL(url) );
	...
	public static void AppsItemListenersLoad()
	{
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
//		Debug.Log("AppsItemListenersLoad");
	...
	// -
	//-------------------------------------------------------------------------------------------------------------------------------
	public static tagAppsItem FindAppsItem( string inpast )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		if( (inpast)==(null) ) return (null);
		...
		foreach( AppsItemListener listerner in Data().AppsItemListeners )
		{
			foreach( tagAppsItem appsitem in listerner._GetList() )
			{
				if( (appsitem.id)==(inpast) )
				{
					return (appsitem);
				}
			}
