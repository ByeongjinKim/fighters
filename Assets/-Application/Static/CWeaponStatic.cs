...
//무기 정보를 처리하기 위한 클래스
public class CWeaponStatic : AppsItemListener
{
	private List<tagWeaponStatic> Weapons = new List<tagWeaponStatic>();

	protected override void Awake()
	{
		base.Awake();
		Initialize( null, "Weapons", UPDATE_CLASS.Weapon.ToString(), null );
	}

    //객체를 생성하기 위한 함수 
    protected override tagAppsItem New()
    {
        return new tagWeaponStatic();
    }
	...
	protected override void OnDelete( tagAppsItem appsItem )
	{
		...
		GetList().Remove( appsItem as tagWeaponStatic );
    }

	//리스트를 얻기 위한 함수
	public List<tagWeaponStatic> GetList()
	{
		return Weapons;
	}

	//객체를 검색하기 위한 함수
    public tagWeaponStatic Find( string id )
    {
        if( !Library.Is(id) ) return null;

        foreach( tagWeaponStatic weapon in GetList() )
        {
            if( weapon.id==id )
            {
				return weapon;
...
//		if( col==null ) return;	//(NULL)값을 허용함
		...
		tagWeaponStatic weapon = appsItem as tagWeaponStatic;
